...
class TinkoffParser(BaseParser):
    ...
    def parse(self, file_bytes: bytes, filename: str) -> list[ParsedBooking]:
        ...
        for _, row in df.iterrows():
            row = row.fillna("")
            booking_id = self._find(row, ["номер", "id брони", "booking id", "номер заказа"])
            if not booking_id:
                continue
            guest = self._find(row, ["гость", "клиент", "имя", "фио", "guest"])
            check_in = self.parse_date(self._find(row, ["заезд", "дата заезда", "check-in", "дата заезда"]))
            check_out = self.parse_date(self._find(row, ["выезд", "дата выезда", "check-out"]))
            if not check_in or not check_out:
                continue
            gross = self.clean_amount(self._find(row, ["сумма брони", "сумма", "стоимость", "amount"]))
            comm_rate = self.clean_amount(self._find(row, ["% вознаграждения", "комиссия %", "commission %"]))
            comm_amt = self.clean_amount(self._find(row, ["вознаграждение", "комиссия", "commission"]))
