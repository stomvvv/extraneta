...
class Gis2Parser(BaseParser):
    ...
    def parse(self, file_bytes: bytes, filename: str) -> list[ParsedBooking]:
        ...
        for _, row in df.iterrows():
            row = row.fillna("")
            booking_id = self._find(row, ["booking_id", "id", "номер брони"])
            if not booking_id:
                continue
            guest = self._find(row, ["guest", "гость", "клиент"])
            check_in = self.parse_date(self._find(row, ["checkin", "check_in", "заезд"]))
            check_out = self.parse_date(self._find(row, ["checkout", "check_out", "выезд"]))
            if not check_in or not check_out:
                continue
            gross = self.clean_amount(self._find(row, ["amount", "сумма", "стоимость"]))
            comm_rate = self.clean_amount(self._find(row, ["commission_rate", "комиссия %", "% комиссии"]))
