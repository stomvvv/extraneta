...
class YandexParser(BaseParser):
    ...
    def parse(self, file_bytes: bytes, filename: str) -> list[ParsedBooking]:
        ...
        for _, row in df.iterrows():
            row = row.fillna("")
            booking_id = self._find(row, ["номер брони", "id брони", "booking_id", "id"])
            if not booking_id:
                continue
            guest = self._find(row, ["гость", "фио", "guest", "имя гостя"])
            check_in = self.parse_date(self._find(row, ["заезд", "дата заезда", "check_in", "check-in", "дата заезда"]))
            check_out = self.parse_date(self._find(row, ["выезд", "дата выезда", "check_out", "check-out"]))
            if not check_in or not check_out:
                continue
            gross = self.clean_amount(self._find(row, ["сумма", "стоимость", "amount", "total", "цена"]))
            comm_rate = self.clean_amount(self._find(row, ["комиссия %", "% комиссии", "ставка комиссии", "commission_rate"]))
            comm_amt = self.clean_amount(self._find(row, ["комиссия", "сумма комиссии", "commission", "вознаграждение"]))
