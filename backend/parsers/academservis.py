...
class AcademservisParser(BaseParser):
    ...
    def parse(self, file_bytes: bytes, filename: str) -> list[ParsedBooking]:
        ...
        for _, row in df.iterrows():
            row = row.fillna("")
            booking_id = self._find(row, ["номер", "id", "booking_id"])
            if not booking_id:
                continue
            guest = self._find(row, ["фио", "гость", "клиент", "guest"])
            check_in = self.parse_date(self._find(row, ["заезд", "дата заезда", "check_in"]))
            check_out = self.parse_date(self._find(row, ["выезд", "дата выезда", "check_out"]))
            if not check_in or not check_out:
                continue
            gross = self.clean_amount(self._find(row, ["сумма", "стоимость", "цена", "amount"]))
            comm_rate = self.clean_amount(self._find(row, ["ставка", "% комиссии", "комиссия %", "commission %"]))
