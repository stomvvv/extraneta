"""
Academservis parser — STUB

⚠️  THIS PARSER IS BASED ON ASSUMPTIONS.
    No real Academservis export file was available. Verify and update COLUMN_MAP before production use.
"""
...
COLUMN_MAP = {
    "booking_id": ["Номер брони", "Бронь", "booking_id", "ID"],
    "guest_name": ["Гость", "ФИО", "Имя гостя"],
    "check_in":   ["Дата заезда", "Заезд", "Прибытие"],
    "check_out":  ["Дата выезда", "Выезд", "Убытие"],
    "room_type":  ["Тип номера", "Категория"],
    "gross":      ["Стоимость", "Сумма", "Итого"],
    "net":        ["К выплате", "Выплата отелю"],
    "commission": ["Комиссия", "Комиссия Academservis"],
...
class AcademservisParser(BaseParser):
    OTA_SOURCE = OTASource.academservis

    def parse(self, file_bytes: bytes, filename: str) -> ParseResult:
        result = ParseResult()
        result.warnings.append("⚠️ Academservis parser is a stub — verify column names against a real export.")
        try:
        ...
        if missing:
            result.errors.append(f"Academservis: missing {missing}. Found: {list(df.columns)}")
            return result
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                result.bookings.append(ParsedBooking(
                    source_ota=OTASource.academservis, booking_id_ota=bid,
