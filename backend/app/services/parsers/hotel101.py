"""
101отель parser — STUB

⚠️  THIS PARSER IS BASED ON ASSUMPTIONS.
    No real 101отель export file was available. Verify and update COLUMN_MAP before production use.
"""
...
COLUMN_MAP = {
    "booking_id": ["Номер брони", "ID брони", "booking_id", "Номер заказа"],
    "guest_name": ["Гость", "ФИО гостя", "Имя гостя"],
    "check_in":   ["Дата заезда", "Заезд", "Дата въезда"],
    "check_out":  ["Дата выезда", "Выезд"],
    "room_type":  ["Тип номера", "Категория номера"],
    "gross":      ["Стоимость", "Сумма брони", "Оплачено"],
    "net":        ["Выплата", "К выплате"],
    "commission": ["Комиссия", "Комиссия 101hotel"],
    "rate":       ["% Комиссии", "Ставка комиссии"],
    "status":     ["Статус", "Статус брони"],
}
...
class Hotel101Parser(BaseParser):
    OTA_SOURCE = OTASource.hotel101

    def parse(self, file_bytes: bytes, filename: str) -> ParseResult:
        result = ParseResult()
        result.warnings.append("⚠️ 101отель parser is a stub — verify column names against a real export.")
        try:
        ...
        if missing:
            result.errors.append(f"101отель: missing {missing}. Found: {list(df.columns)}")
            return result
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                result.bookings.append(ParsedBooking(
                    source_ota=OTASource.hotel101, booking_id_ota=bid,
